...
public class AHBottomNavigation extends FrameLayout {
	...
	private void createClassicItems(LinearLayout linearLayout) {
		...
		for (int i = 0; i < items.size(); i++) {
			...
			icon.setImageDrawable(getTintDrawable(i));
...
//						itemInactiveColor, itemActiveColor, forceTint);
				icon.setImageDrawable(items.get(itemIndex).getSelectedDrawable());
...
//						itemActiveColor, itemInactiveColor, forceTint);
	...
	private Drawable getTintDrawable(int index) {
		final boolean current = currentItem == index;
		if (tintAble) {
			return forceTint ? AHHelper.getTintDrawable(items.get(index).getDrawable(context),
					current ? itemActiveColor : itemInactiveColor, forceTint)
					: items.get(index).getDrawable(context);
		} else {
			return current ? items.get(index).getSelectedDrawable()
					: items.get(index).getDrawable(context);
		}
	}
